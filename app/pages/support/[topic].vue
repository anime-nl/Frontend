  ...
  <div class="max-w-2xl mx-auto p-4 md:p-8 flex flex-col gap-8">
    ...
    <UCard v-else>
      <UForm :state="state" :validate="validateSupportRequest" class="flex flex-col gap-5" @submit="onSubmit">
        ...
        <UFormField name="orderNumber" label="Order number" :required="topic.orderNumberRequired"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <UFormField name="email" label="Email address" required>
            <UInput v-model="state.email" type="email" autocomplete="email" :maxlength="supportLimits.email"
                    class="w-full"/>
          </UFormField>
        </div>

        <UFormField name="message" label="Message" required>
          <UTextarea v-model="state.message" :rows="6" :maxlength="supportLimits.message"
                     :placeholder="topic.messagePlaceholder" class="w-full"/>
        </UFormField>
        ...
        <UAlert v-if="submitError" color="error" variant="subtle" icon="i-lucide-circle-alert"
